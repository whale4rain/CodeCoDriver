...
function App() {
  ...
  const runSuite = async () => {
    ...
    try {
      const response = await fetch('/evaluations/suites', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: 'dashboard suite', mode: 'agent', case_ids: evaluation.cases.map(item => item.id) }) })
      if (!response.ok) throw new Error(`${response.status} ${response.statusText}`)
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      {view === 'evaluation' && <EvaluationView data={evaluation} onRun={() => void runSuite()} />}
    </main>
  </div>
}
