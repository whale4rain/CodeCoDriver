...
function TasksView({ tasks, selected, timeline, reviewReason, setReviewReason, onTask, onReview, onApply, onRerun, applyResult }: {
  tasks: Task[]; selected: Task | null; timeline: TimelineEvent[]; reviewReason: string; setReviewReason: (value: string) => void
  onTask: (task: Task) => void; onReview: (task: Task, approve: boolean) => void; onApply: (task: Task) => void; onRerun: (task: Task) => void; applyResult: { taskId: string; status: string; warnings: string[] } | null
}) {
