...
human_review = [2, 5]
total = [6, 6]
avg_duration_ms = [73450, 86097]
memory_hits = [25, 0]
repair_attempts = [10, 12]
success_hits = [12, 0]
failure_hits = [5, 0]
resolved_hits = [8, 0]
refined_hits = [10, 0]
...
with_results = [True, True, True, True, False, False]
without_results = [False, False, False, True, False, False]


def chart_completion():
    fig, ax = plt.subplots(figsize=(7, 4))
    width = 0.3
    x = range(len(modes))
    ax.bar([i - width / 2 for i in x], completed, width, label="completed", color="#4f9d69")
    ax.bar([i + width / 2 for i in x], human_review, width, label="human review", color="#d8a95f")
    ax.set_xticks(list(x))
    ax.set_xticklabels(modes)
    ax.set_title("Completed vs Human Review - V5", fontweight="bold")
    ax.set_ylabel("Cases")
    ax.legend()
    ax.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
    fig.savefig(CHARTS / "completion.png", dpi=160)
    plt.close(fig)


def chart_pass_rate():
    fig, ax = plt.subplots(figsize=(6, 4))
    rates = [p / c if c else 0 for p, c in zip(passed, completed)]
    bars = ax.bar(modes, rates, color=["#4f9d69", "#d8b45f"])
    ax.bar_label(bars, fmt="%.0f%%", padding=2)
    ax.set_ylim(0, 1.15)
    ax.set_title("Completed-Run Pass Rate - V5", fontweight="bold")
...
def chart_metrics():
    ...
    width = 0.18
    x = range(len(modes))
    values = [memory_hits, repair_attempts, success_hits, failure_hits, resolved_hits, refined_hits]
    labels = ["memory hits", "repair attempts", "success hits", "failure hits", "resolved hits", "refined hits"]
    colors = ["#4f9d69", "#d8a95f", "#5b8f8f", "#c95d43", "#7f9bb3", "#b06f9b"]
...
def chart_per_case():
    ...
    ax.set_title("Per-Case Memory A/B Result - V5", fontweight="bold")
    ax.legend()
    ax.spines[["top", "right"]].set_visible(False)
    fig.tight_layout()
